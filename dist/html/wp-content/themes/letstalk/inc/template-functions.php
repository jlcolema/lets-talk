 ...
 * @return array
 */
function lets_talk_body_classes( $classes ) {
...
add_filter( 'body_class', 'lets_talk_body_classes' );
 ...
 * @return array
 */
function lets_talk_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'lets_talk_post_classes', 10, 3 );
 ...
 * @return void
 */
function lets_talk_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'lets_talk_pingback_header' );
 ...
 * @return void
 */
function lets_talk_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'lets_talk_supports_js' );
 ...
 * @return array
 */
function lets_talk_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'lets_talk_comment_form_defaults' );
 ...
 * @return bool
 */
function lets_talk_can_show_post_thumbnail() {
	return apply_filters(
		'lets_talk_can_show_post_thumbnail',
 ...
 * @return int
 */
function lets_talk_get_avatar_size() {
	return 60;
}
 ...
 */
function lets_talk_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'letstalk' ),
 ...
 */
function lets_talk_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . lets_talk_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'lets_talk_continue_reading_link_excerpt' );

/**
 * Create the continue reading link.
 */
function lets_talk_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . lets_talk_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'lets_talk_continue_reading_link' );

if ( ! function_exists( 'lets_talk_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function lets_talk_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'letstalk' ) : $title;
	}
}
add_filter( 'the_title', 'lets_talk_post_title' );
 ...
 * @return string
 */
function lets_talk_get_icon_svg( $group, $icon, $size = 24 ) {
	return Lets_Talk_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function lets_talk_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? lets_talk_get_icon_svg( 'ui', 'arrow_right' ) : lets_talk_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? lets_talk_get_icon_svg( 'ui', 'arrow_left' ) : lets_talk_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'lets_talk_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function lets_talk_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'lets_talk_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'lets_talk_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'lets_talk_generate_css' ) ) {
	...
	return lets_talk_generate_css( // @phpstan-ignore-line.
 ...
 */
function lets_talk_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function lets_talk_password_form( $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'letstalk' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'letstalk' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'letstalk' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'letstalk' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'lets_talk_password_form' );
 ...
 * @return array
 */
function lets_talk_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'lets_talk_get_attachment_image_attributes', 10, 3 );
